...
final class HomeViewController: UIViewController, UIScrollViewDelegate {
    ...
    var onCreateTabTapped: (() -> Void)?
    var onReturnHomeRequested: (() -> Void)?
    ...
    private func buildBottomControls() {
        tabBar.onHomeTapped = { [weak self] in
            HapticFeedback.selection()
            self?.onHomeTabTapped?()
        }
        tabBar.onCreateTapped = { [weak self] in
            HapticFeedback.selection()
            self?.onCreateTabTapped?()
        }
    ...
    @objc private func openLucas() {
        ...
        Task { @MainActor [weak self] in
            ...
            detailViewController.onReturnHome = { [weak self] in
                self?.onReturnHomeRequested?()
            }
...
private final class BottomNavigationBarView: UIView {
    ...
    private func build() {
        ...
        let home = makeTabButton(imageName: "HomeTabIcon", label: "Home", isSelected: true)
        let create = makeTabButton(imageName: "CreateTabIcon", label: "Create", isSelected: false)
        let chat = makeTabButton(imageName: "ChatTabIcon", label: "Chat", isSelected: false)
    ...
    private func makeTabButton(imageName: String, label: String, isSelected: Bool) -> UIButton {
        let button = UIButton(type: .system)
        button.setImage(tabIconImage(named: imageName)?.withRenderingMode(.alwaysTemplate), for: .normal)
        button.tintColor = isSelected ? .white : UIColor.white.withAlphaComponent(0.40)
